...
## Status: TOOL-VERIFIED 2026-09-05 (twice — see the `genesisTreasury` block) — freezes at the deploy, which has not happened yet
...
called a layout change before the compiler did rather than after. **One unpredicted change
has landed since**: `windowAnte` took slot 37 and `__gap` is now `uint256[9]` at 38. It
took the slot correctly and told nobody — see the 2026-09-05 entries below, both for the
layout and for why "no storage-affecting change remains expected" is a claim this document
should stop making until the deploy closes it.
...
Slots 13, 18, 21, 23, 26, 33 and 38 all have free bytes. **Do not fill them** — packing a new
