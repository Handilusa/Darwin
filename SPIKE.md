...
| 4 | The precompile's real callback selector, and the 11-parameter `subscribe(...)` ABI | **CLOSED — `onEvent(address,bytes32[],bytes)`**, from `SomniaEventHandlerABI` in the installed `@somnia-chain/reactivity@0.2.1`. `onSomniaEvent` does not exist anywhere in the SDK; it was this repo's placeholder. The `REACTIVITY_CALLBACK_SIG` escape hatch is gone from `SelectionEngine`; `subscribe.ts:132` still honours it as an env override against a future SDK change, and setting it wrongly is now the one remaining way to build a subscription that can never fire. The 11-field tuple is confirmed by live reads: `getSubscriptionInfo(1)`/`(2)` decode cleanly against it on Shannon. Checkpoint §2.2–2.3. |
...
| 9 | ~~Constrained inference with a non-empty `allowedValues`~~ | **CLOSED 2026-09-07 — it is served.** Half closed on 2026-08-29 by simulation (a payload carrying all nine values was accepted, which only proved it is not rejected). Closed for real by a census of **182 of our own live requests**: every one of the **78 Successes** carried the nine values `Genome.allowedBeliefs()` emits, and one carried **27**, so the constraint is neither rejected nor silently truncated at nine. The census also names what had actually been broken, which was never `allowedValues`: `perAgentReward`. With `chainOfThought` held **false** throughout, **78/78 Success at 0.07 STT per agent and 0/104 at 0.001** separates all 182 — the escrow floor from `getRequestDeposit` prices a request validators are entitled to decline, and they declined it. The two failure modes are indistinguishable from outside, because a dropped constraint and an unserved request both produce a population that abstains every window. `Believed` carrying **zeroed validator addresses** is what told them apart: that is `status != Success`, so `Genome.parseAnswer` had never run and the genomes were innocent. `contracts/src/interfaces/ISomnia.sol:20-26`, Checkpoint §3. |
