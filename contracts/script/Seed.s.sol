    ...
    function _report(Population population, string[] memory names, uint256 n) internal {
        ...
        for (uint256 i; i < n; ++i) {
            ...
            json = string.concat(
                ...
                '"id": ', vm.toString(id), ", ",
                '"name": "', names[i], '", ',
                '"address": "', vm.toString(address(p)), '", ',
                '"genomeHash": "', vm.toString(p.genomeHash()), '", ',
                '"generation": ', vm.toString(uint256(p.generation())), ", ",
                '"birthWindow": ', vm.toString(uint256(p.birthWindow())),
                i + 1 == n ? "}\n" : "},\n"
            );
        }
